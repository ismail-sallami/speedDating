...
$userdetails = fetchUserDetails(NULL, NULL, NULL, $userId); //Fetch user details
	...
	else
	{
		...
		if(isset($_POST['activate']) && $_POST['activate'] == "activate"){
			if (setUserActive($userdetails['activation_token'])){
				$successes[] = lang("ACCOUNT_MANUALLY_ACTIVATED", array($userdetails['user_name']));
			}
			else {
				$errors[] = lang("SQL_ERROR");
			}
		...
		if ($userdetails['title'] != $_POST['title']){
			...
			if(minMaxRange(1,50,$title))
			{
				...
				if (updateTitle($userId, $title)){
					$successes[] = lang("ACCOUNT_TITLE_UPDATED", array ($userdetails['user_name'], $title));
				}
				else {
					$errors[] = lang("SQL_ERROR");
				}
		...
		$userdetails = fetchUserDetails(NULL, NULL,NULL, $userId);
	}
}
